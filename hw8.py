'''
Edited on October 31, 2019
@author:Kai Qi Chee (for the homework problems)
...
def wordScoreLoop(S, scorelist):
    ...
    for i in range(len(S)):
        a=a+int(letterScoreLoop(S[i],scorelist))
    return a
...
def wordsWithScoreLambda(dct, scores):
    ...
    a= map(lambda x: [x, wordScoreLoop(x, scores)], dct)
    return a
...
def wordsWithScoreLoop(dct, scores):
    ...
    for i in range(len(dct)):
        u=u+[[dct[i],wordScoreLoop(dct[i],scores)]]
    return u
