...
def rod_cutter(values, n):
    '''returns the highest added value that can be given for a specific n'''
    if values ==[]:
        return 0
    if n<=0:
        return 0
...
def rod_cutter_with_values(values, n):
    '''returns the highest added value thatcan be given for a specific n, with the list of lengths'''
    if values ==[]:
        return (0, [])
    if n<=0:
        return (0, [])
        ...
        use_it=rod_cutter_with_values(values,n-values[0][0])
        lose_it=rod_cutter_with_values(values[1:],n)
        uval=values[0][1]+use_it[0]
        lval=lose_it[0]
    if uval>lval:
        return (uval, use_it[1]+[values[0][0]])
    else:
        return lose_it
